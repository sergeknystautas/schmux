...
- **Exhausted label, not a misleading deficit.** When a window reports `used_percent >= 100` and its `resets_at` is still in the future, the row replaces the reserve/deficit math with the `Limit reached` label, rendered in the same danger color as `deficit`. The horizon and reset countdown remain. Exhaustion requires a future reset; a missing `resets_at` or a missing `used_percent` keeps the existing `N/A` display instead. The threshold is inclusive (`>= 100`) and unrounded; a reported `150%` is exhausted, not a `150% deficit`. Exhaustion does not require a known duration: an eligible unknown-duration window (e.g. Codex `primary`) renders the exhausted label and its existing countdown in place of an `N/A` balance.
- **Provider-local suppression of shorter exhausted windows.** Within a single provider card, the panel omits displayable windows whose known duration is strictly shorter than the longest exhausted known-duration window in that card. Equal-duration and longer windows stay visible; windows with unknown duration stay visible and do not trigger suppression (their relationship to the exhausted window cannot be established). The filter is purely a display decision over the existing displayable set — `provider.windows` and the snapshot are not mutated, and the API schema is unchanged. Ineligible buckets (filtered out by `isDisplayableWindow` already) cannot suppress anything.
- **Restoration on reset.** When an exhausted window's `resets_at` passes, the row returns to `Awaiting update` with `0h left` and shorter rows in the same card become visible again on the next render. Restoration depends on the panel's shared clock tick — even when the refresh request fails, the elapsed clock alone is enough to release suppression and flip the label. A subsequent report with `used_percent < 100` on the same window restores shorter rows before the old reset time, through the existing snapshot-replacement path.
...
- `UsedPercent` is a `*float64` (pointer) for a reason: a missing utilization must not be coerced to 0. The frontend renders `N/A` for reserve/deficit when either the duration, used percentage, or reset timestamp is missing, and `N/A` for time remaining when the reset is missing — it never invents a zero. A missing duration suppresses pace calculation but does **not** suppress the exhausted label for an otherwise eligible window; an eligible unknown-duration window still renders `Limit reached` with its existing countdown.
- An expired window (reset timestamp in the past) renders `Awaiting update` and `0h left`. Do not show a fabricated reserve for the next window; the next live report is what resets the panel. Exhausted windows follow the same rule — once their reset time passes they revert to `Awaiting update`, not a continuation of the exhausted label.
- Kimi's durationless monthly bucket (`limit_month_total`) is intentionally ineligible: it has no displayable duration and no fallback in `windowDurationMinutes`, so it is dropped by `isDisplayableWindow` and never appears as an exhausted row. The duration rule applies only inside the displayable set — eligibility, not just exhaustion, gates whether a window can be labeled or rendered at all.
...
- **To change the exhausted label or the suppression rule:** edit `isExhaustedWindow` and `ProviderCard` in `PlanUsagePanel.tsx`. The shared predicate is private and used by both labeling (in `WindowBalance`) and selection (in `ProviderCard`) — keep them in sync. The threshold is `used_percent >= 100` with a future `resets_at`; do not require a known duration. Suppression is strictly local to a single provider card and strictly less-than the longest exhausted known duration; preserve input order, preserve unknown durations, and never mutate `provider.windows`.
