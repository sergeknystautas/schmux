...
#### Render completion API

The completion API (`TerminalStream.waitForRenderSettled`) waits on a
semantic boundary rather than elapsed time. The atomic event is the
`terminal.write` callback inside xterm.js's `WriteBuffer`: per
`@xterm/xterm` 6.0.0, `_innerWrite` invokes the parser for a chunk and
then runs that chunk's callback FIFO before the next chunk. A callback
therefore fires only after that write's data — and everything queued
before it — has been parsed and is reflected in `buffer`. The public
typings state the same contract.

`waitForRenderSettled` accepts one of three conditions:

- `{ marker }` — the marker string parsed into the terminal buffer (scanned
  in the same window `waitForSentinel` used: `baseY-50 … baseY+rows`).
- `{ bootstrapComplete: true }` — the `bootstrapComplete` control message
  was seen.
- `{ resizeApplied: true }` — resize debounce drained and new dims recorded.

The predicate holds when the condition is true **and** the pipeline is
clean: `writeBuffer === ''`, no rAFs pending (`writeRAFPending`,
`scrollRAFPending`, `viewportSyncRAFPending`), no `writeGuardTimer`, no
`gapRequestPending`, and no `resizeDebounceTimer`. The 15-second default
timeout is a failure backstop, not a correctness gate — `waitForRenderSettled`
never rejects, it resolves `{ ok: false, timedOut: true }` with
diagnostics.

`resetAndSettle()` is drain-then-reset: queue a barrier write
(`terminal.write('', cb)`) and wait for its callback (the ordered parse
barrier guarantees everything submitted before has parsed), cancel the
app's pending write-flush rAF, clear `writeBuffer`/`pendingWriteCb`, call
`terminal.reset()`, void outstanding waiters with `{ ok: false,
reason: 'reset' }`. Without the drain, xterm's `WriteBuffer` does not
clear queued-chunk state on `reset()` — already-submitted but not-yet-parsed
data would land in the fresh buffer.

The marker is generated by `sendTmuxCommandWithSentinel` as a prompt-embedded
string (PS1 reassignment), not as a literal echoed command — the echoed
command line never contains the contiguous marker, so it cannot match
echoed input. Re-awaits of an already-satisfied marker resolve immediately
because the helper evaluates the condition on registration.
