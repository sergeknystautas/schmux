...
**Mixer re-test (2026-06-15 spike — FAIL; the mixer does not escape the Room-coupling gate).** The 2026-06-14 read above missed `AudioManager.shared.mixer.capture(appAudio:)`, the documented app-audio feed, so it was re-tested on its own as this spike's kill gate. The mixer IS a public, Room-free, non-mic PCM feed — `capture(appAudio:)` (`Audio/MixerEngineObserver.swift:337`) plus `setManualRenderingMode(true)` (`Audio/Manager/AudioManager.swift:366`) schedule app PCM onto `appNode`, a player node in the ADM's `AVAudioEngine` (`MixerEngineObserver.swift:345-358`). So the "no custom-PCM source" and "mic is the source" bullets above are incomplete — there is a non-mic feed. But `appNode.engine` is the engine of `RTC.peerConnectionFactory.audioDeviceModule` (`Core/RTC.swift:74`; the mixer is wired as its engine observer in `AudioManager.init`, `AudioManager.swift:498`) — the same internal factory as the first bullet. It therefore hits the identical gate: the rtc audio track (`Track.mediaTrack`, `Track/Track.swift:93`) is internal, peer creation (`RTC.createPeerConnection`, `Core/RTC.swift:78`) is internal and `Room`-only, and a foreign factory's peer can't carry the track (WebRTC's same-factory rule). `@testable import LiveKit` is not a way out — the helper ships as a release binary (`swift build -c release`), and testability requires `-Onone`, so the internals are unreachable in the signed helper regardless. The mixer is a real non-mic feed, but it cannot reach a helper-owned peer.

So the SDK's default audio capture path is mic-backed, and — as the mixer re-test confirms — its factory/peer are `Room`-coupled and internal for _every_ feed, including the non-mic mixer. Path #1 cannot deliver Room-decoupled PCM on a helper-owned peer — the candidate is closed, not unproven, now confirmed against the one mechanism the earlier read missed. **Decision — a custom `AudioDeviceModule` on a webrtc build we own.** Audio delivered as a real track, the documented mechanism. Every _prebuilt_ Apple webrtc gates `RTCAudioDevice` out on macOS (LiveKit hides it; stock builds compile it iOS-only), so we build libwebrtc ourselves with the hook exposed — we own the factory and peer, which satisfies the same-factory rule that closed Path #1. The data-channel route (PCM off-track + Web Audio) is **rejected**: it delivers audio as not-audio, a hack. Committed spec + riskiest-first plan (the build is the gate): `2026-06-15-desktop-streaming-audio-custom-adm-design.md`. Whether SCK audio needs its own TCC grant beyond Screen Recording is still open, answered by that spike.
