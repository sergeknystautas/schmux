...
- **`buildConfigUpdate` also sends `network`, `access_control`, and tmux blocks unconditionally** — populated with values it read from the API GET response, which calls getters that default-fill empty fields (e.g., `tmux_socket_name: "schmux"`, `provider: "github"`, `bind_address: "127.0.0.1"`). The first save of a fresh install therefore mutates those previously-nil-or-empty fields into populated structs holding the same effective value. The restart-needed check at `internal/dashboard/handlers_config.go` is built around this: it compares a `restartRelevantSnapshot` taken before and after the apply, populated via the same getters the runtime uses. Nil/empty/explicit-default all collapse to the same snapshot value, so the form's auto-save round-trip is a true no-op. **If you add a new restart-relevant getter, add it to `snapshotRestartRelevant` and `TestRestartFlag_NotSetOnFirstSaveWithUnchangedFields` will catch any drift.**
