...
- **Nudge clearing on user input.** When the user presses Enter, Tab, or bare Escape in a terminal session, the nudge is automatically cleared via `clearNudgeOnInput()`. Chat sessions do not clear on user input: the headless chat runtime owns the Nudge field and computes the value from the current record and the user's most recent action. A failed input write (e.g., disk full) leaves the pending request pending instead of clearing.
- **Chat sessions are guarded against status events.** `Server.HandleStatusEvent` returns immediately for sessions where `IsChat()` is true; `checkInactiveSessionsForNudge` skips chat sessions at the eligibility check (they have no terminal tracker for NudgeNik to classify from, and the headless runtime owns the field). The status Stop gate's hook script (`stop-status-check.sh`) also short-circuits for chat when `SCHMUX_SESSION_KIND=chat` is set, so the agent is not asked to write a status event. The server-side guard is necessary because already-running chat processes may not have been spawned with the new env marker.
