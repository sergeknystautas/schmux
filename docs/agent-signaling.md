...
│  Agent outputs: printf '\x1b]777;notify;completed;Done\x07'     │
...
Schmux uses the standard OSC 777 notification format:
...
1. Agent outputs OSC 777 sequence to stdout
2. tmux captures output via pipe-pane to log file
3. Schmux WebSocket reads log file, detects OSC sequence
...
- **Invisible** - Signals are stripped from terminal output before display
...
def signal_schmux(state: str, message: str = ""):
    if os.environ.get("SCHMUX_ENABLED") == "1":
        print(f"\x1b]777;notify;{state};{message}\x07", end="", flush=True)
...
function signalSchmux(state, message = "") {
    if (process.env.SCHMUX_ENABLED === "1") {
        process.stdout.write(`\x1b]777;notify;${state};${message}\x07`);
    }
}
...
```go
// Parse OSC 777 signals from terminal output
...
2. In the terminal, run: `printf '\x1b]777;notify;completed;Test signal\x07'`
...
### Non-Schmux OSC 777 Signals

Only OSC 777 notifications with valid schmux states are processed. Other OSC 777 notifications (from other tools) pass through unchanged.
