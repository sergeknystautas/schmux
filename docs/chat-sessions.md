...
- **Per-session drafts and focus in `localStorage`, keyed by session id.** `chat-draft.ts`, `chat-answers.ts`, and `chat-focus.ts` all use the same `localStorage` pattern (key `chat-{draft|answers|focus}-{sessionId}`). Drafts survive switching to another session and back, page reloads, and browser restarts, and are shared across browser tabs — but never cross sessions. Keys are session ids; workspace never appears in the key, so two sessions in the same workspace are isolated too. `ChatSessionPage` is the sole place these stores are read and written, so removing or replacing the persistence is a single page change.
