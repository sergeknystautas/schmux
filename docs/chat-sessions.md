...
| `internal/dashboard/websocket_chat.go`                        | `/ws/chat/{id}`: history (with `protocol`, `last_seq`; only records after `?since=N` on resume) then live records; client frames incl. `answer` and `abort`                                                                                         |
...
| `internal/dashboard/handlers_restart.go`                      | Rejects protocol changes and prepares destination-compatible harness history before Restart                                                                                                                                                         |
...
| `assets/dashboard/src/components/chat/Composer.tsx`           | Textarea + Attach + Send; focus, draft persistence, Enter/Shift+Enter, image paste, file attach via picker / paste / pane drop; sole owner of attachment state; exposes `attachFiles` and `onAttachmentAvailabilityChange` through `ComposerHandle` |
| `internal/dashboard/handlers_attachments.go`                  | `POST /api/workspaces/{id}/attachments?filename=` — raw-file workspace attachment store (≤50 MiB) and the path used by Composer                                                                                                                     |
...
| `assets/dashboard/src/lib/chat/socket.ts`                     | Client side of `/ws/chat/{id}`; threads the protocol and resume metadata from the history frame, sends `since` on each connect                                                                                                                      |
| `assets/dashboard/src/lib/chat/conversation-cache.ts`         | Per-session durable `Conversation` + `lastSeq` kept for the SPA lifetime; the base a resumed history applies to                                                                                                                                     |
| `assets/dashboard/src/lib/chat/types.ts`                      | Wire types for record / frame / model; `ChatProtocol`, `Question.id`, `UserSegment`                                                                                                                                                                 |
| `assets/dashboard/src/lib/chat-draft.ts`                      | Per-session composer drafts in `localStorage`                                                                                                                                                                                                       |
| `assets/dashboard/src/lib/chat-answers.ts`                    | Per-session question-card answer drafts in `localStorage`; cleared delivery-authoritatively                                                                                                                                                         |
| `assets/dashboard/src/lib/chat-focus.ts`                      | Per-session focus record in `localStorage` (composer caret or question target); restored on return                                                                                                                                                  |
| `assets/dashboard/src/lib/chat-scroll.ts`                     | Per-session transcript scroll record in `localStorage` (`bottom`, or a pixel `scrollTop`); restored once history loads                                                                                                                              |
...
- **Restart history preparation is shared and destination-driven.** Every local Restart asks the session manager to prepare the captured harness conversation for the effective target after the old process stops, so the history snapshot cannot race a final append. Harnesses without compatibility rules reuse the captured id. Codex reads the source rollout through app-server metadata and derives the active model-visible history (the latest compacted replacement plus later response items). If the effective destination is OpenAI, plaintext third-party reasoning items are omitted from a newly created continuation because OpenAI cannot resolve their provider-owned ids. The source Codex thread and its complete provider-specific history are not edited. If no item is incompatible, no continuation is created and Restart uses the existing id.

- **Codex needs addressing state that Claude does not.** `turn/start`, `turn/interrupt`, and the responses to server requests carry ids that the next request must use. The Codex `Protocol` keeps the thread and account state learned during the handshake, the active turn, submitted `turn/start` request ids that have not answered yet, and the next client request id. `Addressable` is true only after the handshake and while neither an active turn nor an unanswered turn request exists. `Rebuild` scans `in.jsonl` first to restore submitted current-lifetime requests and client ids, then replays `out.jsonl` through `Observe`; a crash after the input write but before the response therefore cannot release another queued message. The instance is per-runtime (one per session) and the runtime serializes every encode and observe under `Runtime.mu`; implementations hold no locks of their own.
...
- **Queued means held by schmux.** Claude and Codex both admit one user message to the harness at a time. A follow-up accepted while that message's turn is active stays in `Runtime.held`; the dashboard marks the exact message `queued` until its successful harness-input write. Each terminal result releases only the next message, so a burst visibly drains in order. Codex's pending `turn/start` request closes the gate before `turn/started` arrives. Historical Codex native-steer records still render as in-turn user segments, while daemon-held messages move after the completed turn and receive their own assistant turn.

- **The chat-sessions flag is a single switch.** `chat_sessions` (bool) in config, exposed on `GET /api/config`, settable through `POST /api/config`, and toggled on the Settings Advanced tab. The spawn wizard shows the Chat checkbox only when the flag is on and every selected target resolves to a runner whose capabilities include `chat`. The server enforces the same rule.

- **Per-session drafts and focus in `localStorage`, keyed by session id.** `chat-draft.ts`, `chat-answers.ts`, `chat-focus.ts`, and `chat-scroll.ts` all use the same `localStorage` pattern (key `chat-{draft|answers|focus|scroll}-{sessionId}`). Drafts survive switching to another session and back, page reloads, and browser restarts, and are shared across browser tabs — but never cross sessions. Keys are session ids; workspace never appears in the key, so two sessions in the same workspace are isolated too. `ChatSessionPage` is the sole place these stores are read and written, so removing or replacing the persistence is a single page change.

- **Transcript scroll restore lives in `ChatTranscript`, not the page.** The record is `bottom` (the user was following the tail; they return to the new bottom) or `position` with the container's pixel `scrollTop`, the same value the markdown and HTML viewers store. `ChatTranscript` reports the record from its scroll handler through `onScrollChange` and restores `initialScroll` once per mount in a `useLayoutEffect` gated on `historyLoaded`, because restore must clear `atBottomRef` before the conversation effect's animation-frame pin and the `ResizeObserver` callback run, and must happen before paint. The page stays the only reader and writer of the store. Reconnects flip `historyLoaded` again while the DOM position is already right, so a ref makes the restore run once. Cached image previews reserve their dimensions before lazy loading; other content that changes height after restore (such as tool rows that were expanded and render collapsed) shifts the position by that height, the same limitation the document viewers have.
...
- **One Composer, three attach entry points.** `Composer` owns the draft, image data, uploaded-file references, attachment processing, and attachment errors. The picker, the paste handler, and the pane-wide drop target all converge on the same `attachFiles(selectedFiles)` operation inside `Composer`; that operation re-checks `disabled` / `attaching` before doing anything, so a stale "active" event from a drop cannot bypass the guard. `ChatView` does not decide upload eligibility — it asks the Composer via `onAttachmentAvailabilityChange`, and `Composer`'s `useLayoutEffect` reports `!disabled && !attaching` whenever either changes. The view consumes that signal without inspecting the button DOM, which keeps a single disabled/busy gate.

- **The pane-wide drop target is `ChatView`, not the composer.** Sidebar, session tabs, toolbar, and dialogs sit outside it; the toolbar's session tabs and the page chrome must not activate the drop outline. Inside the pane, a depth counter (`fileDragDepthRef`) prevents flicker when crossing child elements (a `dragenter` from a message followed by a `dragleave` from the parent does not clear the overlay). The overlay is an absolutely-positioned `data-testid="chat-file-drop-overlay"` with `pointer-events: none` — it never steals focus, never moves the transcript's scroll position, and never reflows the composer. Text and link drags are not handled: the browser gets them.

- **Escape cancels a drag before it interrupts a turn.** When a file drag is active, the first Escape clears the drag feedback (resets the depth counter, hides the overlay) without firing `onInterrupt`. The second Escape interrupts the running turn. The order matters: cancelling a drag must not stop a long-running answer the user only meant to abandon the drop for.

- **Pane drops do not auto-send.** A drop calls `attachFiles` exactly once, with the same validation, upload behavior, progress indicator, and error display as the picker. The user's draft text and any prior attachments survive; successful attachments appear in the existing composer chip row. The `disabled` and `attaching` states both reject drops without uploading, queueing, or navigating the browser to the file.
...
- **Codex patch snapshots can dwarf the transcript.** The dashboard ignores `turn/diff/updated`, so the chat WebSocket omits those records from history and live delivery. Completed `fileChange` items keep their full patch on `item/completed`; history trims the repeated patch from `item/started`. The persisted conversation log is unchanged, and the reducer's tool input stores only file paths and change kinds because the patch is already the tool result.

- **Reconnects resume from a durable browser cache, not from rendered state.** `conversation-cache.ts` holds, per session and for the SPA lifetime (a page reload drops it), the conversation reduced from durable records only, plus the `last_seq` it reached. Each connect sends `?since={lastSeq}`; the daemon numbers durable records 1..N on every read (the number is never written to `conversation.jsonl`) and returns only the suffix, and `useChatSocket` folds that suffix onto the cached conversation. Live `record` frames and queue overlays update the rendered conversation but never the cache, so a reconnect deliberately drops stale live-only streaming state, exactly as a full replay would. A frame with `reset: true` or a history frame without `last_seq` (an older daemon during a rolling reload) takes the cold path: reduce from an empty conversation. Sequences are positions in the parsed log, so anything that rewrites or truncates `conversation.jsonl` under a running daemon invalidates every browser cache for that session.

- **Chat load measurements use a dedicated local file.** The dashboard sends each load and image sample to `POST /api/chat/telemetry`; the daemon appends structured `history`, `image`, `browser_load`, and `browser_image` events to `~/.schmux/diagnostics/chat-performance.jsonl`. The `load_id` in a history frame joins the server and browser events for one load. The Advanced `Chat Load Profiling` toggle enables source-versus-sent record categories and outliers, reducer time by category, conversation size, and separate image lookup, generation, transfer, and post-response times. The file survives daemon restarts and excludes message and image contents. The browser reads and clears samples left by older tab-storage code after it successfully uploads them. New samples are never stored in the tab.
...
- **Composer drafts are stored in `localStorage`, per the repo convention.** Drafts survive switching to another session and back, reloads, and browser restarts, per session; sending clears it; another session's draft never appears in this one. Lowering the persistence scope to memory or `sessionStorage` loses drafts on reload. (The spawn wizard's draft is the exception: it stays in `sessionStorage`, scoped to the tab.)
...
- **Nudge replay follows the current lifetime and successful input writes.** Records before the most recent session-ended marker are history only. Control records are reconciled against the input file by ordered occurrence and applied at their original record position, so failed answers/interrupts do not become successful after a daemon restart. Only the final snapshot is published. Before the first Claude `user_message_dispatch` marker, the legacy isReplay echo distinguishes the active message from queued follow-ups (including identical text) so Done cannot land between queued turns; from the marker forward, schmux owns the queue and the `isReplay` echo is display history only. Child execution and telemetry cannot finish or restart a root turn; unrecoverable harness errors produce Error, while retry notifications and tool failures alone do not.
...
- **Held sends are derived, not persisted.** The runtime records the `user_message` immediately and holds it whenever the protocol is not addressable: during Codex's handshake, while either harness has an active turn, or while Codex is waiting for a submitted `turn/start` response. Codex allocates the request id only at flush time. After a daemon restart, `protocol.Rebuild` scans the input file for current-lifetime `clientUserMessageId`s and unanswered turn requests, replays the output, and returns each recorded user message that never reached the input as held. Claude uses a durable `user_message_dispatch` marker before the input write; rebuild reconciles markers by exact-line matching so a failed write remains held without duplicating a successful one. The dashboard observes the shared queue through live-only `user_message_queue` records: a subscription snapshot marks every current `held` record queued, and each successful harness-input write clears exactly that message.

- **Claude legacy takeover waits for schmux state, not `queued_turn_count`.** A surviving pre-marker Claude process may still own native-queue work after a result reports zero queued turns. Runtime records a `claude_takeover` marker before accepting new sends on that process. On restore, it replays Nudge state up to that marker and holds new sends until the legacy state reaches Idle, Completed, or Error. Messages accepted after the marker do not enter legacy Nudge accounting; Nudge sees each one when schmux actually dispatches it, and the first confirmed dispatch marker starts normal daemon-held accounting.
...
- **The drag-target depth counter must be paired with `onDragEnd`.** `dragend` clears the counter for drops that complete (the `drop` handler resets it) and for drags cancelled by the OS (a native Escape, a drag that returns to its source window). Without it, a cancelled desktop drag can leave `fileDragInside` true after the browser has already torn down the drag, so the overlay stays visible until the next event. Same depth rule applies if you ever add nested drop targets inside `ChatView`: the counter is the only thing keeping the overlay from flickering on child crossings.

- **Drag-overlay pointer-events must stay `none`.** The overlay covers the conversation, the activity area, and the composer. If a future styling pass enables pointer events on the dashed outline to give it a hover affordance, drops will land on the overlay node instead of the pane, the depth counter will tick on the overlay first, and child crossings will flicker again. The overlay is a status surface, not an interactive one — leave `pointer-events: none`.

- **Do not add a second `attachFiles` bypass.** Every entry point (picker, paste, pane drop) calls into `Composer`'s `attachFiles`, which is the only place that rechecks `disabled` / `attaching` and only place that owns the upload endpoint, size limit, and error display. A future entry point (URL paste, drag-from-history, an MCP source) must call `ComposerHandle.attachFiles` or `attachFiles` directly; a new upload path that bypasses the guard will let a disabled drop navigate the browser to the file.
...
- **To add a new client-side draft to a chat session.** Mirror `chat-answers.ts` / `chat-focus.ts`: a `localStorage` lib keyed by session id, with a `load<Thing>` / `save<Thing>` pair and corrupt-JSON tolerance. Wire it from `ChatSessionPage` only. If the draft must be cleared on a server-observed event, expose another `resolves<Thing>` from `lib/chat/reducer.ts` and consume it via `useChatSocket`'s callback — never clear on click.
...
- **To add a new attachment entry point (URL paste, drag from history, MCP source).** Do not touch the picker, the `onPaste` handler, or `ChatView`'s drop target. Call `composerRef.current?.attachFiles(files)` (or wrap your source in `Iterable<File>` and call `ComposerHandle.attachFiles`). The handle is the contract: `attachFiles` re-checks `disabled` / `attaching` and reports its progress via `onAttachmentAvailabilityChange`, which `ChatView` already consumes. If the source cannot produce `File` instances (e.g. a remote URL), materialize it through the existing upload endpoint in `Composer.tsx` and add a wrapper around `uploadWorkspaceAttachment` — never invent a parallel upload path.

- **To widen the pane drop target or add a second one.** The target is the `ChatView` root; nested drop targets inside the pane will fight the depth counter. If you need different behavior over a subregion (e.g. drop a file onto a tool result), make that subregion its own React subtree with `stopPropagation` on its own dragenter / dragover / drop, and decide at the parent whether to consume the event. Reuse `onAttachmentAvailabilityChange` so the overlay still reflects the same disabled/busy state.
