...
- `presets` opt into core-defined bundles with additional capabilities or
  ...
  `sentry` (a `sentry` PATH shim exporting `NODE_USE_ENV_PROXY=1`/`SENTRY_CLI_NO_TELEMETRY=1` so the Sentry CLI routes through fence's proxy, plus `sentry.io`/`us.sentry.io`/`de.sentry.io`),
