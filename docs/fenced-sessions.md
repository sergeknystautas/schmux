...
Schmux starts from an embedded copy of Fence's `code` template (`internal/fence/baseline.jsonc`, written to the launch dir as `baseline.jsonc` and referenced by `extends`); the copy is byte-for-byte `code` except that `*.sentry.io` is removed from `deniedDomains`, because fence merges denials append-only and checks them before the allowlist, which would otherwise make the `sentry` preset's region hosts unreachable. A live test diffs the copy against the installed fence's template so drift is a red test, not a stale sandbox. The Fence guide recommends using the `code` template for coding agents, allowlisting only the network destinations needed, and enabling monitor mode to audit blocked attempts.
...
```json
{
  "extends": "/Users/me/.schmux/fence/<workspace>/<session-id>/baseline.jsonc",
  "network": {
...
The remaining behaviors are opt-in via the repo's `fence.presets`, because they grant additional capabilities or change tool behavior beyond ordinary cache redirects:
...
- `sentry`: puts a `sentry` shim on `PATH` (in the per-session launch dir, like `netlify`) and allows `sentry.io`, `us.sentry.io`, and `de.sentry.io` — Sentry's "Dashboard and API" hosts, exact, no wildcard. The Sentry CLI is a Node single-executable whose API client uses native `fetch`, which ignores `HTTPS_PROXY` unless `NODE_USE_ENV_PROXY=1` is set, so without the shim its requests never reach fence's proxy and fail with `Network error` / `fetch failed` beside bare `network-outbound (sentry:<pid>)` denials. The shim exports that flag and `SENTRY_CLI_NO_TELEMETRY=1` (the CLI's crash reporting is off rather than its ingest host allowlisted), then execs the host's real `sentry` by absolute path. Skipped when sentry is not on the host; the hosts are added either way. A repo that needs another Sentry host (a custom domain, a new region) adds it to its own `allowed_domains`. The CLI keeps its state in `~/.config/sentry/`, which the `code` template already allows. On macOS the preset also adds two Sentry Cocoa dirs to `allowWrite`, recursively. `~/Library/Caches/io.sentry`: a native app built with Sentry Cocoa creates its working dir `io.sentry/<hash>` (one per DSN) at SDK init and fails to start with `Failed to create Sentry SDK working directory` when that mkdir is denied. `~/Library/Caches/SentryCrash`: the crash handler writes `SentryCrash/<CFBundleName>/Reports/*.json` when the app crashes and the next launch sends it as the fatal event; denied, the app still crashes but no fatal event ever arrives. Cocoa resolves both from Foundation's user caches dir, which ignores `HOME`, so a test that isolates `HOME` still lands there. Other `Caches` entries stay unwritable, including the SDK's `INSTALLATION` id file (read once it exists) and `<bundle-id>/async.log` (the crash handler's own error log; denial is nonfatal). Security cost: the fenced agent can use the user's stored Sentry login through the CLI, and can read, modify, or delete the offline envelopes and crash reports of any Sentry Cocoa app on the host. The preset does not export `CFFIXED_USER_HOME` (an earlier version did, which hid every Foundation user-domain dir — including other presets' Application Support grants — from the whole session).
...
- `spine`: read/write access to the Spine editor's per-user state dir (`~/Library/Application Support/Spine` — `os.UserConfigDir()/Spine`), recursively, so a fenced session can run the Spine CLI (e.g. sprite-sheet exports). It also allows the regional licensing hosts `us.esotericsoftware.com`, `usapi.esotericsoftware.com`, `eu.esotericsoftware.com`, `euapi.esotericsoftware.com`, `jp.esotericsoftware.com`, and `jpapi.esotericsoftware.com`, because the launcher validates licenses before exports. Spine's launcher rewrites that whole tree on every start: it opens `spine.log` without checking `fopen`'s result (a denied write there ends in SIGSEGV inside `JNI_CreateJavaVM`, before Spine's own code runs), rotates `settings/start-N.json` to `.bak`, creates UUID-named temp files in the dir root, and chmods the dir — a literal `spine.log`-only grant was spiked and the launcher aborted on the settings rotation, so the directory grant is the narrowest one that launches. Security cost: selecting `spine` lets the process modify all persistent per-user Spine state, including settings and any credentials or launcher metadata stored there, but grants no broader home access, no writes inside `Spine.app`, and no kernel-device (IOKit) access — Spine's denied `AppleNVMeEANUC` and `IOHIDParamUserClient` opens and its `Spine.app/Contents/MacOS/shared` writes are nonfatal (a live export succeeds with all of them denied) and stay denied.
- `netlify`: read/write access to the Netlify CLI's global config dir (`~/Library/Preferences/netlify` on macOS — the CLI's `envPaths('netlify', {suffix: ''})`, which is _not_ `os.UserConfigDir()` there; `$XDG_CONFIG_HOME/netlify` or `~/.config/netlify` elsewhere), recursively, plus `api.netlify.com` and `api.netlifysdk.com` (the extensions API `@netlify/config` queries unconditionally for any site with an ID during `netlify deploy`'s build-config resolution; denied, the deploy aborts with `Failed retrieving extensions for site …: fetch failed` before the build runs) and `*.netlify.app`, where every deploy lands — the site itself (`<site>.netlify.app`) and its deploy-preview/branch URLs (`deploy-preview-N--<site>`, `<branch>--<site>`), all one left label — so a fenced session can fetch what it just deployed without per-deploy config edits. Cost of the wildcard: that label space is shared by every Netlify-hosted site, so the grant reaches all of them, not just the project's own. The CLI rewrites `config.json` there on every start — even for `netlify --version` — by creating a randomly named `config.json.tmp-<hex>` sibling and renaming it into place; a denied create is fatal (`EPERM: operation not permitted`, before any command logic or network I/O), so no `allowed_domains` entry can help, and a literal `config.json` grant cannot cover the random temp name. Reads there already pass the `code` template, so only the write is newly granted. The preset also puts a `netlify` shim on `PATH` (in the per-session launch dir, like `vercel`) that exports `NODE_USE_ENV_PROXY=1`: the CLI's own API client reads `HTTP_PROXY`, but `@netlify/config`'s site-info lookup and the telemetry child process build node-fetch clients with no proxy agent and dial directly, which in a fence without unix sockets fails at DNS (`getaddrinfo ENOTFOUND api.netlify.com`) midway through `netlify deploy` — after the deploy ID was already obtained through the proxy. The flag makes Node's core `http`/`https` honor the env proxy so every request routes through fence's proxy; child node processes inherit it, and no preload is needed because node-fetch passes no explicit dispatcher. Skipped when netlify is not on the host. Security cost: `config.json` holds the user's Netlify auth token, so selecting `netlify` lets the process rewrite (not just read) that credential and the CLI's per-user settings; it grants no sibling `Preferences` dir and no other home path. Telemetry (`cli.netlify.com`, `analytics.services.netlify.com`) stays denied and is nonfatal.
...
- `macos-gui`: `macos.mach.lookup` and `macos.mach.register` set to `*`, plus `macos.iokit.userClientClasses` set to `AGXDeviceUserClient`, so a fenced session can run a native windowed app (GUI test suites that need real pixels — see [Security note](#security-note-macos-gui-preset)). AppKit needs Mach IPC to the window server and a wide, macOS-version-dependent set of system services; the baseline denies them, so a windowed app dies during AppKit init (e.g. windowed Godot on `com.apple.hiservices-xpcservice`) before its own code runs. Mach access alone is not enough to draw: fence's baseline allows only the IOSurface and RootDomain IOKit user clients, so opening the GPU device is denied and `MTLCreateSystemDefaultDevice()` returns `nil`. `AGXDeviceUserClient` is the exact class macOS names in the denial, and granting it is what makes a Metal device available and shaders compile. A windowed app typically also needs its per-user data dir writable (Godot: the `godot-editor` preset). No effect on non-macOS platforms.
...
- `vercel`: puts a `vercel` shim on `PATH` (in the per-session fence launch dir, not the workspace) so the Vercel CLI reaches the network through fence's proxy, and allows `vercel.com`/`api.vercel.com`. The CLI passes an explicit undici `dispatcher` to native `fetch` whose callback shape current Node rejects (`InvalidArgumentError: invalid onError method`), killing every request before any network I/O — `monitor.log` stays quiet and the evidence is the CLI's own stderr. The shim exports `NODE_USE_ENV_PROXY=1` (Node's native fetch then honors the proxy env fence sets), `NO_UPDATE_NOTIFIER=1` (suppresses the npm-registry update ping instead of allowlisting it), and appends `--require <preload>` to `NODE_OPTIONS`; the preload drops the explicit `dispatcher` from `fetch` calls so Node's own proxy-aware dispatcher handles them. Every request still goes through fence's proxy and `allowed_domains` enforcement — the preset grants no direct networking and no filesystem capability — and telemetry/update endpoints stay denied. The shim is written by the unfenced daemon into the per-session launch dir (readable via `allowRead`; the fenced session has no write grant there, so neither a repo nor a fenced session can pre-position or tamper with it) and is skipped when vercel is not on the host.
...
Enabling the `macos-gui` preset sets `macos.mach.lookup` and `macos.mach.register` to `*`, which switches off Mach IPC isolation for the fenced session: the process may talk to any macOS system service (pasteboard, Apple Events, accessibility, etc.). The filesystem and network fences remain fully active, and TCC consent prompts still apply. `*` is deliberate rather than a curated service list: the set AppKit needs is wide and shifts across macOS versions, and fence never logs _allowed_ Mach lookups, so a too-narrow list can only be debugged one relaunch at a time. It was validated empirically — a windowed Godot game under fence fails AppKit init with the baseline, and gets a window with this grant. Enable it only for repos whose test suite genuinely drives a native GUI.

The preset also grants one IOKit user client, `AGXDeviceUserClient`. A user client is a direct interface to a kernel driver, so this is a wider grant than a Mach lookup: the sandboxed process is talking to the GPU, and any kernel-side vulnerability in that driver becomes reachable from inside the fence. It is the price of rendering — a Metal renderer cannot start without it.

Unlike the Mach grant, this list stays exact and always will. `macos.iokit.userClientClasses` has no wildcard form: fence validates each entry against `^[A-Za-z][A-Za-z0-9_]*$` and refuses to launch otherwise, so the preset cannot widen into "any device" and the generated Seatbelt profile never contains a bare `(allow iokit-open)`. Keeping the list honest is also practical rather than aspirational, because IOKit denials **are** logged: a missing class appears in `monitor.log` by name (`iokit-open-user-client <class>`) and can be added deliberately. This was measured, not assumed — with mach `*` granted and no IOKit grant, `MTLCreateSystemDefaultDevice()` returns `nil` and the log names `AGXDeviceUserClient`; adding exactly that class yields a device and compiles a shader, while an out-of-workspace write and a non-allowlisted host stay denied.

### Fence build requirement: the macos-gui GPU grant

`macos.iokit.userClientClasses` does not exist in upstream fence. Through v0.1.66 the only macOS knobs are `macos.mach.lookup` and `macos.mach.register`; the Seatbelt template hardcodes a three-entry `(allow iokit-open ...)` list (`IOSurfaceRootUserClient`, `RootDomainUserClient`, `IOSurfaceSendRight`) with no way to extend it. **schmux therefore runs a patched fence build on macOS**, maintained locally until the change lands upstream.

What the patch adds, on branch `feat/macos-iokit-user-client-classes` (7 files, ~295 lines, mostly tests):

- `internal/config/config.go` — the `macos.iokit.userClientClasses` field and its validator. Entries must match `^[A-Za-z][A-Za-z0-9_]*$`, which rejects `*` and makes the value unable to carry quotes, parens, or newlines into the generated profile.
- `internal/sandbox/macos.go` — an `IOKitUserClientClasses` param that emits a second `(allow iokit-open ...)` block listing each class. It has no wildcard branch, so unlike a `*` Mach pattern it can never collapse into a bare `(allow iokit-open)`.
- `internal/sandbox/monitor.go` — `shouldShowViolation` now passes `iokit-` denials through. Stock fence filters them, which is why a missing GPU grant presents as `MTLCreateSystemDefaultDevice()` silently returning `nil` with an empty monitor log.

**Running stock fence instead is silent, not loud.** Fence parses settings with plain `json.Unmarshal` and no `DisallowUnknownFields`, so an unpatched binary ignores the `macos.iokit` block rather than failing the launch: `macos-gui` sessions come up looking healthy and windowed apps fail to render, with no denial logged.

Rebuilding and installing the patched binary:

```bash
cd <fence-checkout> && go build -o /tmp/fence-patched ./cmd/fence
rm /opt/homebrew/Cellar/fence/<version>/bin/fence
cp /tmp/fence-patched /opt/homebrew/Cellar/fence/<version>/bin/fence
codesign --force --sign - /opt/homebrew/Cellar/fence/<version>/bin/fence
```

`rm` before `cp` matters: overwriting in place invalidates the signature and the binary is `SIGKILL`ed at exec. `brew upgrade fence` replaces the binary and silently reverts GPU access, so re-apply after any fence upgrade. `fence -v` reports `dev` for a local build, which is how to tell the two apart.
...
- `presets` opt into core-defined bundles with additional capabilities or
  tool-specific behavior. Available: `golang` (GOCACHE, STATICCHECK_CACHE,
  ...
  `sentry` (a `sentry` PATH shim exporting `NODE_USE_ENV_PROXY=1`/`SENTRY_CLI_NO_TELEMETRY=1` so the Sentry CLI routes through fence's proxy, plus `sentry.io`/`us.sentry.io`/`de.sentry.io`, and on macOS a write grant for Sentry Cocoa's `~/Library/Caches/io.sentry` and `~/Library/Caches/SentryCrash`),
  `godot-editor` (read/write the Godot editor config dir
  `~/Library/Application Support/Godot`), `spine` (read/write the Spine editor
  state dir `~/Library/Application Support/Spine`, plus the six regional
  Esoteric Software licensing hosts), `chromium` (macOS Mach lookup/register
  grants for `org.chromium.*`, required to run Chromium — even headless — while
  fenced), `macos-gui` (Mach lookup/register `*` plus the `AGXDeviceUserClient`
  IOKit user client, required to run a native windowed app that renders — see
  [Security note](#security-note-macos-gui-preset)), `swift`
  (a `swift` PATH shim adding `--disable-sandbox` to `build`/`test`/`run`, so
  SwiftPM's nested sandbox does not abort fenced builds), `vercel` (a `vercel`
  PATH shim + Node preload in the per-session launch dir that routes the Vercel
  CLI through fence's proxy, plus `vercel.com`/`api.vercel.com`), `netlify`
  (read/write the Netlify CLI's global config dir `~/Library/Preferences/netlify`,
  which it rewrites on every start, a `netlify` PATH shim exporting
  `NODE_USE_ENV_PROXY=1` so its proxy-unaware clients route through fence's
  proxy, plus `api.netlify.com`/`api.netlifysdk.com`/`*.netlify.app`). The
...
The always-on baseline (any fenced repo) is `extends: <launch dir>/baseline.jsonc` (the embedded copy of `code`), the workspace +
...
- The `macos-gui` preset's GPU grant needs a patched fence build — see [Fence build requirement](#fence-build-requirement-the-macos-gui-gpu-grant). On a stock binary the setting is ignored without error and windowed apps fail to render.
