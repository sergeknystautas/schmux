...
- `sentry`: on macOS, creates `<workspace>/.cache/schmux-fence/sentry-home` and exports `CFFIXED_USER_HOME` pointing there. Sentry Cocoa's offline envelopes are then buffered under `sentry-home/Library/Caches/io.sentry/<sha>/`, within the already-writable workspace. Adds no host filesystem or network access; the former grant to the real `~/Library/Caches/io.sentry/` is removed. **Functional cost:** the environment variable redirects all Foundation user-domain directories for the entire session and its child processes, including preferences and Application Support, not only Sentry caches. Existing host settings and data may therefore be absent from Foundation lookups. `HOME` is unchanged. If only one self-test needs isolation, setting `CFFIXED_USER_HOME` narrowly in that self-test is more precise than enabling it for the session. No effect on other platforms. This cache redirect does not grant signal, Mach exception, or task-port capabilities; diagnose native crash handling separately with an outside-versus-inside comparison before considering such grants.
...
- `presets` opt into core-defined bundles with additional capabilities or
  ...
  `sentry` (macOS-only `CFFIXED_USER_HOME` redirect to a workspace-local
  `sentry-home`, affecting all Foundation user-domain directories),
