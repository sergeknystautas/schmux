...
LLM provider variants are preconfigured "skins" over existing AI coding agents that redirect them to alternative API endpoints. They work by setting specific environment variables before running the underlying tool's command.

**Example**: `kimi-thinking` sets `ANTHROPIC_BASE_URL=https://api.moonshot.ai/anthropic` and other environment variables, then runs `~/.claude/local/claude`.
...
Add a new `variants` section to `~/.schmux/config.json`:
...
1. Resolve the `base_tool` to its detected command (e.g., `claude` → `~/.claude/local/claude`)
2. Merge variant `env` with user-provided secrets from `~/.schmux/secrets.json`
3. Prepend environment variable exports to the tmux session command
4. Execute: `cd <workspace> && export FOO=bar && export BAZ=qux && <base_tool> <prompt>`
...
When spawning a session, the agent picker shows:
...
Final tmux command for a variant session:

```bash
cd /path/to/workspace-001 && \
...
~/.claude/local/claude 'implement feature X'
```
...
- Detect if agent is a variant
- If so:
  - Resolve base tool command
  - Fetch secrets from secrets file
  - Build environment export string
  - Prepend exports to tmux command
...
### Spawn Wizard
- Add "Variants" section below detected tools
