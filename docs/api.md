...
- `fence.presets` (string[]) — opt-in fence presets: `golang`, `tmux`, `docker`, `godot-editor`, `chromium`, `macos-gui`, `spine`, `swift`, `vercel`. (npm/pip/Playwright cache redirects are baseline, applied to every fenced session — not presets.)
