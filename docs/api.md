...
- Workspace `ci_status` / `ci_url` / `pr_number` / `pr_url` are populated by the **build monitor check pass** for workspaces whose repo is enabled in the build monitor with a connected identity (`github_login` in `build_monitor.repos`). Absent when the build monitor is disabled, the repo isn't monitored, the workspace is a remote-host workspace, or the branch has no remote. `ci_status` is one of `none` (no runs yet for the remote head), `pending` (a run is expected, queued, or in progress), `failure` (the newest run per workflow shows failure or timed_out), `success` (all matching runs completed successfully). PRs are open only — a merged or closed PR is omitted. The pass uses the repo's per-identity OAuth token, not the `gh` CLI. When git status observes a newly pushed remote head before GitHub exposes its run, the server consults the build monitor unit's repo-level active-workflow list and serves `pending` with no `ci_url`; workflow availability is not copied into workspace status. One missing-run check remains pending, then a second miss for the same head settles to `none` so branch/path-filtered workflows do not pulse indefinitely. `pr_number`/`pr_url` are unaffected while CI is pending.
