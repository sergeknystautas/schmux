...
2. **Remote sessions**: The remote signal watcher currently reads a per-session file via `inotifywait`. The per-session event file is the same granularity, so the watcher script adapts naturally — it watches the event file instead of the signal file. The sentinel-marker approach for control-mode output parsing remains the same.

3. **Daemon restart recovery**: On restart, the daemon scans `.schmux/events/*.jsonl` in each workspace, reads the latest `status` event from each file, and cross-references with known sessions in state. Files for sessions that no longer exist in state can be cleaned up.
