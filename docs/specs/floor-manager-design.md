...
- **Left column:** Agent status strip (top) + floor manager terminal (fills remaining space)
...
A compact row of agent indicator pills. Each pill shows:

- Color-coded state dot (green = working, yellow = waiting, red = error, grey = completed/stopped)
- Nickname or target name
- Truncated one-liner from latest nudge summary

Tappable to navigate to that agent's session detail page. The floor manager's own session is excluded from the strip.
...
internal/provision/provision.go  # signalCommandWithIntent(),
