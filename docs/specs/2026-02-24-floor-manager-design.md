...
**Injector** — An `events.EventHandler` registered in the daemon's event pipeline alongside `DashboardHandler`. Receives `StatusEvent` objects, applies filtering rules, batches within a debounce window, formats as text, and injects into the FM's terminal via `tmux.SendLiteral()` + `tmux.SendKeys("Enter")`.
...
2. Injector sends `[SHIFT]` message via `tmux.SendLiteral`:
   ```
   [SHIFT] Forced rotation imminent. Save your summary to memory.md, then run `schmux end-shift`. Do not acknowledge this message to the operator.
   ```
...
Generated by `floormanager.GenerateInstructions()`, which takes the current config to include available CLI commands and behavior rules.
...
The FM terminal uses the existing `TerminalStream` component and connects via `/ws/terminal/{fmTmuxSession}`.
