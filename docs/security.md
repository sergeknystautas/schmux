...
- **The raw-file endpoint's extension allowlist guards rendering, not access.** `serveWorkspaceFile` in `handlers_diff.go` restricts inline responses to images, Markdown, Mermaid, HTML, and CSS so the browser never renders arbitrary workspace content. `?download=1` skips that allowlist on purpose: the response is `application/octet-stream` with `Content-Disposition: attachment` and `nosniff`, so nothing is rendered. Every other check (traversal, directory, case-sensitive existence, VCS ignore) runs in both modes; the ignore check comes after the allowlist, so a mode that skips the allowlist must not skip it too. Download mode is rejected for remote workspaces before delegation.
- **Jump and tab navigation use the same VCS-ignore gate as raw-file serving.** `validateWorkspaceFileTarget` performs the workspace, traversal, symlink, regular-file, case-sensitive existence, and VCS-ignore checks before `/jump` redirects or the tab API creates navigation state. A confirmed ignored target returns 403; a failed ignore check returns 500 and never redirects.
