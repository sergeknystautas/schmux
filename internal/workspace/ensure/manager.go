...
// eventWriteCommand returns a shell command that writes a status event to the event file (append JSON).
...
// eventWriteCommandWithIntent returns a shell command that extracts a JSON field from stdin
// and writes a status event with intent to the event file.
// IMPORTANT: No single quotes allowed — command is embedded in JSON then shell-quoted.
func eventWriteCommandWithIntent(state, promptJqField string) string {
	return fmt.Sprintf(
...
// eventWriteCommandWithBlockers returns a shell command that extracts a JSON field from stdin
// and writes a status event with blockers to the event file.
// IMPORTANT: No single quotes allowed — command is embedded in JSON then shell-quoted.
func eventWriteCommandWithBlockers(state, messageJqField string) string {
	return fmt.Sprintf(
		`{ MSG=$(jq -r ".%s // empty" 2>/dev/null | tr -d "\n" | cut -c1-100 || true); `+
			`JMSG=$(printf "%%s" "$MSG" | jq -Rs . 2>/dev/null | sed "s/^.//;s/.$//"); `+
			`[ -n "$SCHMUX_EVENTS_FILE" ] && printf "{\"ts\":\"%%s\",\"type\":\"status\",\"state\":\"%s\",\"message\":\"%%s\",\"blockers\":\"%%s\"}\n" "$(date -u +%%Y-%%m-%%dT%%H:%%M:%%SZ)" "${JMSG}" "${JMSG}" >> "$SCHMUX_EVENTS_FILE"; `+
			`} || true`,
		messageJqField, state,
	)
}
		...
		"SessionStart": {
			{
				Hooks: []claudeHookHandler{
					{
						Type:          "command",
						Command:       eventWriteCommand("working"),
		...
		"SessionEnd": {
			{
				Hooks: []claudeHookHandler{
					{
						Type:          "command",
						Command:       eventWriteCommand("completed"),
						StatusMessage: "schmux: signaling",
					},
				},
			},
		...
		"UserPromptSubmit": {
			{
				Hooks: []claudeHookHandler{
					{
						Type:          "command",
						Command:       eventWriteCommandWithIntent("working", "prompt"),
						StatusMessage: "schmux: signaling",
					},
				},
			},
		...
		"Notification": {
			{
				...
				Hooks: []claudeHookHandler{
					{
						Type:          "command",
						Command:       eventWriteCommandWithBlockers("needs_input", "message"),
						StatusMessage: "schmux: signaling",
					},
				},
			},
