...
// OpenMermaidTab opens a Mermaid diagram preview tab for the given filepath.
func (m *Manager) OpenMermaidTab(wsID, path string) (*state.Tab, error) {
	label := filepath.Base(path)
	tab := state.Tab{
		ID:        uuid.NewString(),
		Kind:      "mermaid",
		Label:     label,
		Route:     "/diff/" + wsID + "/mmd/" + url.PathEscape(path),
		Closable:  true,
		Meta:      map[string]string{"filepath": path},
		CreatedAt: time.Now(),
	}
