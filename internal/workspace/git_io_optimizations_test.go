...
func TestGitStatus_WatcherStillReturnsAccurateLocalState(t *testing.T) {
	...
	dirty, _, _, _, _, _, _, _, _, _, _, currentBranch, _, _ := m.gitStatus(ctx, "ws-test", RefreshTriggerWatcher, cloneDir, remoteDir)
...
func TestGitStatus_ReturnsCurrentBranch(t *testing.T) {
	...
	// On main branch
	_, _, _, _, _, _, _, _, _, _, _, branch, _, _ := m.gitStatus(ctx, "ws-test", RefreshTriggerExplicit, cloneDir, remoteDir)
	if branch != "main" {
...
func TestGitStatus_FilesChangedCountsUntrackedFilesInNewDirs(t *testing.T) {
	...
	_, _, _, _, _, filesChanged, _, _, _, _, _, _, _, _ := m.gitStatus(ctx, "ws-test", RefreshTriggerExplicit, cloneDir, remoteDir)
...
func TestUpdateVCSStatus_NoDuplicateBranchQueries(t *testing.T) {
	...
	// and one to record the remote head SHA (stale-CI-status downgrade).
	// Before the optimization, updateGitStatusWithTriggerAndRound would run an
	// extra rev-parse for branch detection. Verify that duplicate is still gone.
	revParseCount := snap.Counters["git_rev-parse"]
	if revParseCount > 2 {
		t.Errorf("expected at most 2 rev-parse calls (branch detection + remote head SHA), got %d — duplicate query not eliminated", revParseCount)
	}
}
...
func TestCommitsSyncedWithRemote_DerivedFromRevList(t *testing.T) {
	...
	_, _, _, _, _, _, commitsSynced, remoteBranchExists, _, _, _, _, _, _ := m.gitStatus(ctx, "ws-test", RefreshTriggerExplicit, cloneDir, remoteDir)
...
func TestCommitsSyncedWithRemote_FalseWhenAhead(t *testing.T) {
	...
	_, _, _, _, _, _, commitsSynced, _, _, localUnique, _, _, _, _ := m.gitStatus(ctx, "ws-test", RefreshTriggerExplicit, cloneDir, remoteDir)
	if commitsSynced {
