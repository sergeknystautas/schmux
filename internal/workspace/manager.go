...
// These are stored in ~/.schmux/bare/ separate from worktree base repos.
func (m *Manager) EnsureBareClones(ctx context.Context) error {
	...
	for _, repo := range m.config.GetRepos() {
		...
		// Clone as bare repo
		fmt.Printf("[workspace] creating bare clone for branch queries: %s\n", repo.Name)
		if err := m.cloneBareRepoForQueries(ctx, repo.URL, bareRepoPath); err != nil {
...
// cloneBareRepoForQueries clones a repository as a bare clone for branch querying.
func (m *Manager) cloneBareRepoForQueries(ctx context.Context, url, path string) error {
	args := []string{"clone", "--bare", url, path}
	cmd := exec.CommandContext(ctx, "git", args...)
...
func (m *Manager) FetchBareClones(ctx context.Context) {
	bareReposPath := m.config.GetBareReposPath()
	if bareReposPath == "" {
		return
	}
	...
	for _, repo := range m.config.GetRepos() {
		...
		if output, err := cmd.CombinedOutput(); err != nil {
			fmt.Printf("[workspace] warning: fetch failed for bare clone %s: %v: %s\n", repo.Name, err, string(output))
		}
		cancel()
	}
}
