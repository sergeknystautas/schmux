...
func (m *Manager) LinearSyncToDefault(ctx context.Context, workspaceID string) (*LinearSyncResult, error) {
	...
	dirty, ahead, behind, linesAdded, linesRemoved, filesChanged, _, _, _, _, _, _ := m.gitStatus(ctx, workspaceID, RefreshTriggerExplicit, workspacePath, w.Repo)
	if dirty {
		return &LinearSyncResult{
			Success: false,
			Branch:  defaultBranch,
...
func (m *Manager) PushToBranch(ctx context.Context, workspaceID string, confirm bool) (*LinearSyncResult, error) {
	...
	dirty, _, _, linesAdded, linesRemoved, filesChanged, _, _, _, _, _, _ := m.gitStatus(ctx, workspaceID, RefreshTriggerExplicit, workspacePath, w.Repo)
