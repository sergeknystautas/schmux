...
func TestShellQuote(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			input:    `say "hello"`,
			expected: `'say "hello"'`, // Double quotes are literal in single quotes
		},
		{
			name:     "string with single quote",
			input:    "it's working",
			expected: `'it'\''s working'`, // Single quote escaped with '\''
		},
		{
			name:     "empty string",
			input:    "",
		...
		},
		{
			...
			input:    "test;ls",
			expected: `'test;ls'`, // Semicolons are literal, no expansion
		},
		{
			name:     "string with newline",
			input:    "line1\nline2",
			expected: "'line1\nline2'", // Newlines preserved literally
		},
		{
			name:     "string with variable",
			input:    "$HOME/path",
			expected: `'$HOME/path'`, // Variables NOT expanded in single quotes
		},
	}
...
func TestShellQuote_NullByteProtection(t *testing.T) {
	// Null bytes are preserved in single quotes (shell interprets them as string terminators)
	// This is safer than double quotes which allow command substitution
	input := "dangerous\x00command"
	result := shellQuote(input)

	// Single quotes preserve everything, including null bytes
	expected := "'dangerous\x00command'"
	if result != expected {
		t.Errorf("shellQuote(%q) = %q, expected %q", input, result, expected)
	}
}
