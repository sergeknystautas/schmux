...
// FloorManagerSessionInfo holds a snapshot of session info for the floor manager.
// This is a deliberate copy to avoid holding the state lock while callers use the data.
...
func (m *Manager) spawn(ctx context.Context, isRestart bool) error {
	...
	if _, err := m.session.Spawn(ctx, opts); err != nil {
		return err
	}
...
func (m *Manager) checkAndRestart(ctx context.Context) {
	...
	fmt.Printf("[floor-manager] session exited, disposing stale session before restart\n")

	// Dispose the dead session so spawn's singleton guard doesn't block
	// and the nickname is freed for the new session.
	m.session.Dispose(ctx, sess.ID)

	fmt.Printf("[floor-manager] attempting resume-first restart after %s\n", restartDelay)
