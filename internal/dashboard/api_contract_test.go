...
func TestAPIContract_DiagnosticMiddlewareFollowsFeatureConfig(t *testing.T) {
	server, cfg, _ := newTestServer(t)
	...
	tests := []struct {
		...
		{name: "event monitor", panelID: "eventMonitor", middleware: server.eventMonitorMiddleware},
		{name: "tmux diagnostics", panelID: "tmuxDiagnostic", middleware: server.tmuxDiagnosticsMiddleware},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if rr.Code != http.StatusNotFound {
				t.Fatalf("expected 404 with %s off, got %d", tt.panelID, rr.Code)
			}

			cfg.UI.Panels = map[string]bool{tt.panelID: true}
			req = httptest.NewRequest(http.MethodGet, "/test", nil)
			rr = httptest.NewRecorder()
			handler.ServeHTTP(rr, req)
			if rr.Code != http.StatusOK {
				t.Fatalf("expected 200 after enabling %s, got %d", tt.panelID, rr.Code)
			}
		})
	}
}

func TestAPIContract_HealthzHasNoGlobalDebugMode(t *testing.T) {
	server, _, _ := newTestServer(t)
	...
	if _, exists := resp["debug_mode"]; exists {
		t.Fatalf("expected global debug_mode to be absent, got %v", resp["debug_mode"])
	}
}

func TestAPIContract_ConfigUpdatePersistsDiagnosticPanels(t *testing.T) {
	server, cfg, _ := newTestServer(t)

	// Verify default is false
	if cfg.GetEventMonitorEnabled() {
		t.Fatal("Event Monitor should default to false")
	}

	// Enable via config update API
	body := []byte(`{"ui":{"panels":{"eventMonitor":true}}}`)
	req := httptest.NewRequest(http.MethodPost, "/api/config", bytes.NewReader(body))
	rr := httptest.NewRecorder()
	newTestConfigHandlers(server).handleConfigUpdate(rr, req)
	...
	// Verify persisted in live config
	if !cfg.GetEventMonitorEnabled() {
		t.Error("Event Monitor should be true after config update")
	}
	...
	if !resp.UI.Panels["eventMonitor"] {
		t.Error("Event Monitor should be true in GET /api/config response")
	}

	// Disable via config update API
	body = []byte(`{"ui":{"panels":{"eventMonitor":false}}}`)
	req = httptest.NewRequest(http.MethodPost, "/api/config", bytes.NewReader(body))
	rr = httptest.NewRecorder()
	newTestConfigHandlers(server).handleConfigUpdate(rr, req)
	...
	if cfg.GetEventMonitorEnabled() {
		t.Error("Event Monitor should be false after disabling")
	}
}
...
func TestAPIContract_ConfigGetReturnsDiagnosticPanels(t *testing.T) {
	...
	if resp.UI.Panels["eventMonitor"] {
		t.Error("Event Monitor should be false by default in GET /api/config")
	}

	// Enable Event Monitor directly on config
	cfg.UI.Panels = map[string]bool{"eventMonitor": true}
	if err := cfg.Save(); err != nil {
		t.Fatalf("failed to save config: %v", err)
	}
	...
	if !resp.UI.Panels["eventMonitor"] {
		t.Error("Event Monitor should be true in GET /api/config after enabling")
	}
}
