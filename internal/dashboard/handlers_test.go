...
func TestHandleConfigUpdate_FloorManagerToggle(t *testing.T) {
	...
	t.Run("restarts FM when target changes while enabled", func(t *testing.T) {
		...
		var mu sync.Mutex
		var wg sync.WaitGroup
		wg.Add(2) // expect two calls: disable then enable
		server.SetFloorManagerToggle(func(enabled bool) {
			mu.Lock()
		...
		// Wait for both toggle calls
		wg.Wait()

		mu.Lock()
		defer mu.Unlock()
		if len(toggleCalls) != 2 {
			t.Fatalf("expected 2 toggle calls, got %d", len(toggleCalls))
		}
		if toggleCalls[0] != false {
			t.Error("expected first toggle call to be false (disable)")
		}
		if toggleCalls[1] != true {
			t.Error("expected second toggle call to be true (enable)")
		}
	})
