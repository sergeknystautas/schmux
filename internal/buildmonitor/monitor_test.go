...
func TestStatus(t *testing.T) {
	...
	t.Run("disabled → absent", func(t *testing.T) {
		...
		m.recordCommit(repo, branch, "sha1", StatusSuccess, "https://run", true)
		if _, _, ok := m.Status(repo, branch, "sha1"); ok {
	...
	t.Run("empty sha → absent", func(t *testing.T) {
		m := newMon(true, "")
		if _, _, ok := m.Status(repo, branch, ""); ok {
			t.Fatal("want absent")
		}
	})
	...
	t.Run("unrecorded commit + active workflows → queued (first push)", func(t *testing.T) {
		m := newMon(true, "")
		st, _, ok := m.Status(repo, branch, "newsha")
	...
	t.Run("unrecorded commit + repo without workflows → absent", func(t *testing.T) {
		m := newMon(false, "")
		if _, _, ok := m.Status(repo, branch, "s"); ok {
			t.Fatal("want absent")
		}
	})
	...
	t.Run("recorded commit → its status", func(t *testing.T) {
		m := newMon(true, "")
		m.recordCommit(repo, branch, "sha1", StatusSuccess, "https://run", true)
		st, url, ok := m.Status(repo, branch, "sha1")
	...
	t.Run("head moved → queued (post-push gap)", func(t *testing.T) {
		m := newMon(true, "")
		m.recordCommit(repo, branch, "old", StatusSuccess, "https://run", true)
		st, _, ok := m.Status(repo, branch, "new")
		if !ok || st != StatusQueued {
			t.Fatalf("got (%q,%v), want (queued,true)", st, ok)
		}
	})
	...
	t.Run("fork commit lives under the fork repo", func(t *testing.T) {
		...
		m.recordCommit(gh("fork", "app"), branch, "sha1", StatusFailure, "https://forkrun", true)
		st, url, ok := m.Status(gh("fork", "app"), branch, "sha1")
		if !ok || st != StatusFailure || url != "https://forkrun" {
			t.Fatalf("got (%q,%q,%v)", st, url, ok)
		}
		// The same SHA under the base repo must not inherit the fork's status.
		st, _, ok = m.Status(repo, branch, "sha1")
		if !ok || st != StatusQueued {
...
func TestSetDisabled(t *testing.T) {
	...
	m.setEnabled()
	m.setRepoMeta(repo, true, "")
	m.recordCommit(repo, "main", "sha1", StatusSuccess, "u", true)

	if !m.setDisabled() {
		t.Fatal("setDisabled should report held data")
	}
	if _, _, ok := m.Status(repo, "main", "sha1"); ok {
...
func TestPruneExcept(t *testing.T) {
	...
	m.setRepoMeta(repo, true, "")
	m.recordCommit(repo, "main", "sha1", StatusSuccess, "u", true)
	m.recordCommit(repo, "main", "sha2", StatusFailure, "u2", true)

	m.pruneExcept(map[commitKey]bool{{owner: "acme", repo: "app", branch: "main", sha: "sha1"}: true})

	if _, _, ok := m.Status(repo, "main", "sha1"); !ok {
		t.Fatal("referenced commit should survive pruning")
	}
	m.mu.Lock()
	_, present := m.commits[commitKey{owner: "acme", repo: "app", branch: "main", sha: "sha2"}]
	m.mu.Unlock()
	if present {
...
func TestHydrate(t *testing.T) {
	...
	t.Run("snapshot with successful head → success", func(t *testing.T) {
		...
		st, url, ok := m.Status(repo, "main", "h1")
	...
	t.Run("snapshot with failing head → failure", func(t *testing.T) {
		...
		if st, _, ok := m.Status(repo, "main", "h1"); !ok || st != StatusFailure {
	...
	t.Run("repo with no workflows → absent", func(t *testing.T) {
		...
		if _, _, ok := m.Status(repo, "main", "h1"); ok {
