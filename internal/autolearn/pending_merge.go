...
// InvalidateIfContainsLearning deletes the pending merge if any of its LearningIDs matches the given learningID.
func (s *PendingMergeStore) InvalidateIfContainsLearning(repo, learningID string) error {
	s.mu.Lock()
	defer s.mu.Unlock()
