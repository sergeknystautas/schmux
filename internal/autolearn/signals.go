...
// Protected by signalsMu for safe concurrent use with AppendEntry.
...
func PruneEntries(path string, maxAge time.Duration) (pruned int, err error) {
	...
	for _, pl := range parsed {
		...
		// Check if this is a signal entry that should be pruned
		if e.StateChange == "" {
			tsStr := e.Timestamp.Format(time.RFC3339)
			if pruneSet[tsStr] {
				pruned++
	...
	tmp, err := os.CreateTemp(dir, "autolearn-prune-*.tmp")
	if err != nil {
