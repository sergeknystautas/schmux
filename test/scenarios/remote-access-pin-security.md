...
reject them. The server enforces a minimum password length of 8 characters on
...
- POST /api/remote-access/set-password with `{"password": "abc"}` returns 400 with body containing "at least 8"
- POST /api/remote-access/set-password with `{"password": "12345"}` returns 400 (5 chars, still too short)
- POST /api/remote-access/set-password with `{"password": "1234567"}` returns 400 (7 chars, still too short)
