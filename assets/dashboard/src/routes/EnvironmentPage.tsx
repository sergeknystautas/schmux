...
type Tab = 'variables' | 'dependencies';
  ...
  return (
    <>
      <p className="mb-lg text-muted">
        Compare the current system environment against the tmux server environment. Sync individual
        variables so new sessions pick up the changes.
      </p>
...
export default function EnvironmentPage() {
  ...
  const [tab, setTab] = useState<Tab>(
    params.get('tab') === 'dependencies' ? 'dependencies' : 'variables'
  );
  ...
  return (
    <>
      ...
      <div className="wizard__steps" role="tablist" aria-label="Environment views">
        <button
          type="button"
          className={`wizard__step cursor-pointer ${tab === 'variables' ? 'wizard__step--active' : ''}`}
        ...
        <button
          type="button"
          className={`wizard__step cursor-pointer ${tab === 'dependencies' ? 'wizard__step--active' : ''}`}
      ...
      <div className="spawn-content">
        {tab === 'variables' ? <VariablesTab /> : <DependenciesPanel />}
      </div>
    </>
  );
}
