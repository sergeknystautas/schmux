...
### Step 3: Bad Code Check

Run `./badcode.sh` now and wait for it to complete.

**If issues are detected:** STOP. Do not commit. Fix the issues, then re-invoke /commit from the beginning.
...
**If no API-related files are staged, or `docs/api.md` is already staged:** Continue to Step 5.
