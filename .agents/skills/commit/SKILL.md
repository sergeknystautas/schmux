---
name: commit
description: Create a git commit with schmux's definition-of-done enforcement. Use only when the user explicitly invokes this skill or asks to commit the current staged changes.
---
...
- `.agents/**`, `.claude/**` (agent/skill configuration)
...
- Sanitize the staged content (replace the term with a generic placeholder), re-stage, and re-invoke this skill, or
- Confirm the match is a false positive (e.g., a string in a public test fixture that coincidentally matches a pattern), tighten the pattern in their local file, and re-invoke this skill.
...
If candidates are found, ask the user: "This commit appears to complete [feature]. Want me to run the `finalize` skill to consolidate these docs into a subsystem guide?"

- If yes: invoke the `finalize` skill with the candidate file path(s).
